...
const ch11_smoke: [string, string][] = [
  ["11.1.d", "Chapt11/11.1/11.1.d.xhtml"],  // no testable output criteria in spec
  ...
  ["11.1.h", "Chapt11/11.1/11.1.h.xhtml"],  // expects modal message from event handler
  ["11.1.i", "Chapt11/11.1/11.1.i.xhtml"],  // no testable output criteria in spec
  ["11.1.j", "Chapt11/11.1/11.1.j.xhtml"],  // depends on form submission lifecycle
  ...
  ["11.1.r", "Chapt11/11.1/11.1.r.xhtml"],  // depends on submission replace behavior
  ...
  ["11.1.t", "Chapt11/11.1/11.1.t.xhtml"],  // depends on submission replace behavior
...
test.describe("W3C Ch11 [behavioral promoted]", () => {
  ...
  test("11.3.b — 11.3.b xforms-submit-serialize event with submission-body property", async ({ page }) => {
    ...
    const text = await getRenderedText(page);
    expect(text).not.toContain("Prius");
  });
  ...
  test("11.8.1.a — 11.8.1.a name element with value attribute", async ({ page }) => {
    ...
    const text = await getRenderedText(page);
    expect(text).not.toContain("wrongData");
  });
  ...
  test("11.8.2.a — 11.8.2.a value element with value attribute", async ({ page }) => {
    ...
    const text = await getRenderedText(page);
    expect(text).not.toContain("wrongValue");
  });
