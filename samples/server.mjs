...
const port = Number(process.env.SAMPLES_PORT || "8080");
...
function resolveStaticPath(pathname) {
  ...
  const relativePath = normalized === "" ? "sample1/sample1.html" : normalized;
...
const server = createServer(async (request, response) => {
  try {
    ...
    if (saxonJsPath) {
      // TEST-TRACE: expose SaxonJS3 runtime to sample pages that load /Saxon-JS/SaxonJS3.rt.js; helps tests/supplemental/demo-samples-render.spec.ts "samples page renders XForms controls".
    ...
    if (sefPath) {
      // TEST-TRACE: expose shared SEF assets to sample pages that load /sef/saxon-xforms.sef.json; helps tests/supplemental/demo-samples-render.spec.ts "samples page renders XForms controls".
...
server.listen(port, host, () => {
  console.log(`Samples server running at http://${host}:${port}`);
});
